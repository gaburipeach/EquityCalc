...
class TestShowdown(TestCase):
    ...
    def test_is_flush3(self):
        ...
        gen = [Card(5, 3), Card(4, 3), Card(3, 3), Card(14, 3), Card(11, 3), Card(2, 2), Card(6, 3)]
        s = Showdown(None, None)
    ...
    def test_is_straight_flush5(self):
        ...
        gen = [Card(5, 3), Card(4, 3), Card(14, 3), Card(13, 3), Card(2, 3), Card(3, 3), Card(2, 2)]
        s = Showdown(None, None)
    ...
    def test_find_best_pair(self):
        ...
        board = gen[2:]
        s = Showdown(players, board)
        self.assertEqual([BoardScore.pair, 14, 14, 12, 9, 5], s.find_best(players[0]))

    def test_find_best_two_pair(self):
        # Test if method correctly finds the best two pair combo
        gen = [Card(2, 3), Card(2, 2), Card(14, 1), Card(14, 4), Card(12, 3), Card(5, 3), Card(9, 4)]
        players = [Player([gen[1], gen[0]])]
        board = gen[2:]
        s = Showdown(players, board)
        self.assertEqual([BoardScore.two_pair, 14, 14, 2, 2, 12], s.find_best(players[0]))

    def test_find_best_two_pair2(self):
        # Test if method correctly finds the best two pair combo in three pairs
        gen = [Card(12, 3), Card(2, 2), Card(14, 1), Card(14, 4), Card(12, 3), Card(9, 3), Card(9, 4)]
        players = [Player([gen[1], gen[0]])]
        board = gen[2:]
        s = Showdown(players, board)
        self.assertEqual([BoardScore.two_pair, 14, 14, 12, 12, 9], s.find_best(players[0]))

    def test_find_best_three_kind(self):
        # Test if method correctly finds the best three of a kind combo
        gen = [Card(14, 3), Card(2, 2), Card(14, 1), Card(14, 4), Card(12, 3), Card(8, 3), Card(9, 4)]
        players = [Player([gen[1], gen[0]])]
        board = gen[2:]
        s = Showdown(players, board)
        self.assertEqual([BoardScore.three_kind, 14, 14, 14, 12, 9], s.find_best(players[0]))

    def test_find_best_straight(self):
        # Test if method correctly finds the best straight combo
        gen = [Card(14, 3), Card(2, 2), Card(6, 1), Card(7, 4), Card(5, 3), Card(8, 3), Card(9, 4)]
        players = [Player([gen[1], gen[0]])]
        board = gen[2:]
        s = Showdown(players, board)
        self.assertEqual([BoardScore.straight, 9, 8, 7, 6, 5], s.find_best(players[0]))

    def test_find_best_straight2(self):
        # Test if method correctly finds the best straight combo in 7-card straight
        gen = [Card(10, 3), Card(11, 2), Card(6, 1), Card(7, 4), Card(5, 3), Card(8, 3), Card(9, 4)]
        players = [Player([gen[1], gen[0]])]
        board = gen[2:]
        s = Showdown(players, board)
        self.assertEqual([BoardScore.straight, 11, 10, 9, 8, 7], s.find_best(players[0]))

    def test_find_best_flush(self):
        # Test if method correctly finds the best flush combo
        gen = [Card(4, 4), Card(11, 4), Card(6, 1), Card(12, 4), Card(5, 4), Card(13, 3), Card(9, 4)]
        players = [Player([gen[1], gen[0]])]
        board = gen[2:]
        s = Showdown(players, board)
        self.assertEqual([BoardScore.flush, 12, 11, 9, 5, 4], s.find_best(players[0]))

    def test_find_best_flush2(self):
        # Test if method correctly finds the best flush combo in 7 flush cards
        gen = [Card(4, 4), Card(11, 4), Card(6, 4), Card(12, 4), Card(5, 4), Card(13, 4), Card(9, 4)]
        players = [Player([gen[1], gen[0]])]
        board = gen[2:]
        s = Showdown(players, board)
        self.assertEqual([BoardScore.flush, 13, 12, 11, 9, 6], s.find_best(players[0]))

    def test_find_best_full_house(self):
        # Test if method correctly finds the best full house
        gen = [Card(4, 4), Card(4, 3), Card(6, 4), Card(4, 1), Card(5, 4), Card(5, 2), Card(9, 4)]
        players = [Player([gen[1], gen[0]])]
        board = gen[2:]
        s = Showdown(players, board)
        self.assertEqual([BoardScore.full_house, 4, 4, 4, 5, 5], s.find_best(players[0]))

    def test_find_best_full_house2(self):
        # Test if method correctly finds the best full house in 2 three kinds
        gen = [Card(4, 4), Card(4, 3), Card(6, 4), Card(4, 1), Card(5, 4), Card(5, 2), Card(5, 1)]
        players = [Player([gen[1], gen[0]])]
        board = gen[2:]
        s = Showdown(players, board)
        self.assertEqual([BoardScore.full_house, 5, 5, 5, 4, 4], s.find_best(players[0]))

    def test_find_best_full_house3(self):
        # Test if method correctly finds the best full house in full house + pair
        gen = [Card(4, 4), Card(4, 3), Card(6, 4), Card(6, 1), Card(5, 4), Card(5, 2), Card(5, 1)]
        players = [Player([gen[1], gen[0]])]
        board = gen[2:]
        s = Showdown(players, board)
        self.assertEqual([BoardScore.full_house, 5, 5, 5, 6, 6], s.find_best(players[0]))

    def test_find_best_four_kind(self):
        # Test if method correctly finds the best four of a kind combo
        gen = [Card(4, 4), Card(4, 3), Card(4, 1), Card(4, 2), Card(5, 4), Card(5, 2), Card(5, 1)]
        players = [Player([gen[1], gen[0]])]
        board = gen[2:]
        s = Showdown(players, board)
        self.assertEqual([BoardScore.four_kind, 4, 4, 4, 4, 5], s.find_best(players[0]))
    ...
    def test_find_best_straight_flush2(self):
        # Test if method correctly finds the best straight flush from 7-card straight flush
        gen = [Card(14, 3), Card(11, 3), Card(12, 3), Card(13, 3), Card(10, 3), Card(9, 3), Card(8, 3)]
        players = [Player([gen[1], gen[0]])]
        board = gen[2:]
        s = Showdown(players, board)
        self.assertEqual((True, [14, 13, 12, 11, 10]), s.is_straight_flush(gen))
